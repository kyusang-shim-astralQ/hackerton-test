...
1. `normalize_dict_keys`: 모든 키 UPPER + 재귀, 충돌 시 deep merge. **★ `_SECTION_PARAMETERS_`(밑줄 양쪽, AI 플랜이 내는 변종)는 정식 `SECTION_PARAMETERS`로 정규화**한다 — 안 하면 `fuzzy_correct` 미스로 섹션 파라미터 값이 프루닝/literal 렌더되어 유실됨(이후 §A-3에서 `@param`으로 승격되어 `&SECTION VALUE`로 렌더). 예: `&XC_FUNCTIONAL/_SECTION_PARAMETERS_ PBE` → `&XC_FUNCTIONAL PBE`.
...
- 키에 공백 있으면 `k, param` 분리(예 `KIND H` → k=KIND, param=H). **★ `_DUPL_N` 접미사는 `fuzzy_correct` 전에 `base_k.split("_DUPL_")[0]`로 벗긴다**(반복 섹션 정식명엔 `_DUPL_` 없음). 안 벗기면 `REPLICA_DUPL_1`이 미인식 → force_sync 프루닝에서 **유효 반복섹션(NEB REPLICA 등)이 통째로 제거**됨. `_DUPL_` 정보는 원본 raw 키에 남아 `actual_key` 단계에서 재사용.
- **섹션/키워드 판정**: 값이 dict면 섹션. 아니면 현재 경로 스키마에 keyword로 있으면 키워드, 없고 `sections`에 있으면 섹션.
- `fuzzy_correct(k, current_path)`(alias_map → 현재경로 keyword/sub_section/sections 확인)로 정식명. 없으면: `@`로 시작(전처리 지시문)이면 보존; **그 외엔 스키마(cp2k_input.xml) 어디에도 없는 LLM 환각이므로 `[PRUNE] Unknown section`/`[PRUNE] Unknown` 후 `continue`로 항상 제거**(★ **force_sync 여도 환각은 보존하지 않는다**). 과거엔 `force_sync`면 보존했는데, 첫 제출(retry0)이 `force_sync=True`라 환각 키워드(예 `ABSORPTION_SPECTRUM`)가 검증 게이트를 우회해 .inp에 새어 CP2K abort를 냈다 → force_sync 의미는 "사용자/플랜 옵션 보존"이지 "환각 통과"가 아니다.
...
- **★ 재배치 거리 가드(섹션)**: best 가 현재 경로와 **무관한 가지**(공유 prefix 가 `len(current_path)-1` 미만)면 그 먼 곳으로 옮기지 말고 `[PRUNE]` 후 `continue`. 안 그러면 같은 이름이 스키마 다른 곳에 존재할 때(예 `TDDFPT/PRINT/SPECTRUM` → `RIXS/XAS_TDP/PRINT/SPECTRUM`) **단일 오배치 섹션이 무관한 섹션 트리(+필수 키워드 `DONOR_STATES/ATOM_LIST`)를 통째로 fabricate** 해 CP2K가 `Value requested ... ATOM_LIST`로 abort 한다. 근접 재배치(자식/형제)는 허용.
...
4. **SUBSYS**:
   ...
   - CELL: `periodic = mandatory.periodic or atom_info.periodic or 'XYZ'`. **★ 단, 플랜이 명시적으로 `DFT.POISSON.PERIODIC == 'NONE'`(고립분자 판단)이면 `periodic='NONE'`으로 맞춘다**(안 그러면 기본 XYZ가 LLM의 비주기 판단을 덮어씀; 이후 physics_rules가 POISSON/솔버를 NONE+MT로 일관 동기화). 그 다음 `CELL.PERIODIC = periodic`; `CELL.ABC = f"{c0:.10f} {c1:.10f} {c2:.10f}"`(★ **소수 10자리**); `atom_info.full_cell_text`에서 `re.search(r'ALPHA_BETA_GAMMA\s+([\d\.\s]+)')` 매치 시 `CELL.ALPHA_BETA_GAMMA = m.group(1).strip()`.
...
6. **OT vs DIAGONALIZATION**: `kpts_val = mandatory.kpoints`; `valid_kpts = kpts_val and str(kpts_val).strip().upper() not in ["","NONE","NULL","GAMMA","GAMMA-POINT"]`; `has_kpts = 'KPOINTS' in dft or valid_kpts`; **★ `is_tddfpt = run_type=='TDDFPT' or 'TDDFPT' in dft or _tree_has_key_containing(fe,'TDDFPT')`** — TDDFPT는 `FORCE_EVAL/PROPERTIES` 하위라 `dft`만 보면 영원히 탐지 실패(→ OT 강제로 plan의 DIAGONALIZATION이 덮어써짐). 반드시 FORCE_EVAL 트리 전체에서 탐지. `use_smear is True`(엄격 True). 이 중 하나면 → **DIAG**(`scf.setdefault('DIAGONALIZATION',{})`, `scf.pop('OT')`; **★ `is_tddfpt`이고 `ADDED_MOS` 없으면 `scf['ADDED_MOS']=str(max(NSTATES,20))`** — 가상 궤도 확보(plan 규칙: TDDFPT엔 DIAGONALIZATION+ADDED_MOS 필수). `NSTATES`는 `fe.PROPERTIES.TDDFPT.NSTATES`에서 읽되, **데모용 고정 `_DEMO_TDDFPT_NSTATES=5`가 설정돼 있으면 `TDDFPT.NSTATES`를 그 값으로 덮어씀**(빠른 데모; 운영 전환 시 `None`이면 plan 값 유지). has_kpts·not tddfpt면 `KPOINTS.SYMMETRY='F'` + valid_kpts면 `SCHEME='MONKHORST-PACK '+k`(이미 MONKHORST면 그대로)). 아니면 `scf_algo`(또는 트리 OT/DIAG, 없으면 OT). **OT면** `scf.OT.setdefault('MINIMIZER','DIIS')`, `setdefault('PRECONDITIONER','FULL_SINGLE_INVERSE')`, `scf.pop('DIAGONALIZATION')`, `scf.pop('MIXING')`. **DIAG면** `scf.setdefault('DIAGONALIZATION',{})`, `scf.pop('OT')`.
...
**A-6. `get_manual_snippet(token)`**: forward에서 경로 끝이 token인 섹션/해당 token 키워드를 찾아 `### [SECTION] path / Keywords / Sub-sections` 또는 `### [KEYWORD] ... Default/Enums/Type` 스니펫 최대 5개. (f2 플랜·자가치유 프롬프트의 `xml_context`로 쓰임.) **★ 외부 노출 2가지를 반드시 둘 다 제공**: 모듈 싱글톤 `engine = CP2KSchemaEngine()` + 모듈함수 `def get_manual_snippet(token)` (= `engine.get_manual_snippet(token)`). f2는 `from app.shared.schema_engine import engine`(또는 모듈함수)로 호출 — `from ... import schema_engine`(존재하지 않는 이름) 금지(그러면 xml_context가 빈 문자열이 됨, be/03 회귀버그).
...
- [ ] **★ 환각 0 (스키마 감사)**: 환각 토큰(예 `ABSORPTION_SPECTRUM`·`ANGLE_TOLERANCE`)을 섞은 플랜으로 생성해도 `force_sync=True`에서 전부 프루닝되고, 렌더된 .inp의 모든 `&SECTION`/키워드가 cp2k_input.xml의 유효 경로다(섹션은 부모의 sub_sections, 키워드는 그 경로 keywords에 존재). 오배치 섹션은 먼 가지로 fabricate 되지 않는다(SPECTRUM→RIXS/XAS_TDP 금지).
- [ ] **★ 물성 정합**: TDDFPT(absorption/emission)면 OT가 아니라 **DIAGONALIZATION + ADDED_MOS**가 들어가고 KPOINTS는 제거(Gamma 전용)됨. plan이 DIAGONALIZATION을 냈으면 OT로 덮어쓰지 않음.
