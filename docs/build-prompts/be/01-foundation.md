...
6. **실행 설정**: `backend/`에서 `uvicorn app.main:app --reload --port 8000`로 기동. `.env.example`에 `CLAUDE_API_KEY=` + 클러스터 변수(`USE_SGE`/`CLUSTER_HOST`/`CLUSTER_PORT`/`CLUSTER_USER`/`CLUSTER_PASSWORD`/`CLUSTER_REMOTE_ROOT`/`CLUSTER_QUEUE`/`CLUSTER_PE`/`CLUSTER_MPI_RANKS`/`CP2K_ROOT`/`CP2K_DATA_DIR`/`CP2K_MPIEXEC`/`CP2K_SETVARS`)를 **키만(값 비움)** 나열. `requirements.txt`(fastapi, uvicorn, anthropic, ase, python-dotenv, pydantic, **paramiko**, **lxml**(schema_engine XML 파싱)).
