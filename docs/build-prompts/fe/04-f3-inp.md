...
1. **AI 워크플로 검토**: wizard-store의 `steps[]`(f2 결과)를 가변 N-스텝 타임라인/체크리스트로 표시. 각 스텝 **제외 토글**, run_type·objective(목표) 등 요약, 💡전문가 팁(expert_tip) 인라인. **★ 제외 상태는 cross-feature로 보존돼야 한다(권장: 토글이 `planResult.steps[i].exclude=true`(또는 `selected=false`)를 store에 직접 기록).** 그래야 `/generate-inp`뿐 아니라 **step-5(f4) 제출/모니터에서도** 같은 규칙(`selected!==false && exclude!==true`)으로 걸러진다. 별도 `excludedSteps` 오버라이드 맵을 쓴다면 **반드시 persist 대상에 포함**(fe/01 §6)하고 step-5가 그 맵을 읽게 한다 — 한 곳(f3)에서만 반영하고 step-5가 원본 `steps`를 쓰면 제외 스텝이 되살아난다(실제 버그).
...
### 완료 정의 (DoD)
- [ ] `steps[]`가 편집 가능한 타임라인으로 표시, **제외 토글이 `/generate-inp`뿐 아니라 step-5 제출/모니터에도 반영**(제외 상태가 store에 cross-feature로 보존됨; 새로고침해도 유지).
