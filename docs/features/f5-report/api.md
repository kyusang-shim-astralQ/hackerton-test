...
```jsonc
{
  ...
  "excitations": [ /* absorption/emission(TDDFPT)에서만 */ ],  // [{state,energy_ev,wavelength_nm,osc_strength,is_dark,region}], 에너지 오름차순
  "spectrum": { "wavelengths": [], "intensities": [], "sigma_ev": 0.1 }  // absorption/emission에서만. Gaussian 곡선
}
```
> `excitations`/`spectrum`는 **absorption/emission(TDDFPT) 물성에서만** 포함된다(다른 물성·다중-CIF엔 부재). 백엔드는 `.out`의 `TDDFPT|` 표를 `PHYSICS_PATTERNS["excitation"]`(6캡처: 상태·energy(eV)·x·y·z·strength f)로 **전부** `re.findall` → 각 행 `energy_ev=g[1]`, `osc=g[5]`, `wavelength_nm=1239.84/energy_ev`, `is_dark=osc<1e-4` → `excitations[]`(에너지 오름차순). Gaussian σ=0.1 eV를 300–950 nm/2 nm 그리드에 합산 → `spectrum`. 프런트가 리포트 맨 끝에 흡광 스펙트럼 차트+들뜸 표로 렌더. `summary.target_property`는 최강 피크 한 줄 유지.
